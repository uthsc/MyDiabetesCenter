<?php
$pagename = 'Our Stories';
include("template-top.php");
?>
<div id="content" class="row">
    <div class="columns">
        <h1><?php echo $pagename; ?></h1>

        <div id="ruthie-tate" class="row panel">
            <h2>Ruthie's Story</h2>
            <div class="columns small-12 medium-3 show-for-small">

                <figure class="bio">
                    <img src="images/ruthie-tate.jpg" alt="Picture of Ruthie Tate"/>
                    <figcaption>
                        <strong>Ruthie Tate</strong><br/> Patient Advisory Committee Member
                    </figcaption>
                </figure>
            </div>
            <div class="column small-12 medium-9">
                <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Morbi tincidunt ante id luctus scelerisque. Phasellus ut luctus ipsum, at volutpat ipsum. Nulla et ipsum nibh. Vestibulum efficitur, elit a sollicitudin maximus, ante dolor maximus sem, eu porta nibh massa at nulla. Donec rhoncus porttitor mauris quis condimentum. Nullam malesuada consectetur congue. Aliquam pretium magna venenatis nulla vulputate, ut semper mauris condimentum. Fusce sit amet nisi ullamcorper, maximus nibh ac, pellentesque risus. Integer id malesuada nisi. Praesent sit amet ante ac velit maximus blandit pretium eget diam. In ut sapien eleifend, ultricies purus ut, auctor risus.</p>

                <p>Cras blandit quam sit amet enim pulvinar, eget laoreet tortor sollicitudin. Duis et justo a erat elementum eleifend vel sit amet enim. Duis at luctus dolor. Vivamus fermentum leo nisl. Nam sapien tellus, imperdiet eu lacus ut, commodo volutpat velit. Duis euismod maximus rutrum. Aliquam et mi at metus tincidunt vulputate. Ut ac imperdiet ante, et commodo ante.</p>

                <p>Praesent malesuada tortor eu libero tristique euismod. Sed id risus odio. Quisque dolor massa, facilisis quis nulla id, suscipit hendrerit felis. Nunc semper, libero at rutrum pretium, enim tortor pharetra libero, vitae tempus purus enim et dui. Curabitur vestibulum sapien risus, nec aliquet ante sodales id. Aenean molestie, ligula quis finibus congue, dui turpis egestas turpis, quis placerat eros tellus sit amet purus. Morbi non faucibus ipsum. Mauris vitae augue metus. Mauris sodales ante vel aliquam porttitor. Sed sit amet pellentesque enim. Quisque euismod diam et magna consectetur, at vulputate elit semper.
                </p>
            </div>
            <div class="columns small-12 medium-3 hide-for-small">
                <figure class="bio">
                    <img src="images/ruthie-tate.jpg" alt="Picture of Ruthie Tate"/>
                    <figcaption>
                        <strong>Ruthie Tate</strong><br/> Patient Advisory Committee Member
                    </figcaption>
                </figure>
            </div>
        </div>
        <div id="pastor-william-burks" class="row panel">
            <h2>Pastor William Burks</h2>
            <div class="columns small-12 medium-3">
                <figure class="bio">
                    <img src="images/sihoutte-man.jpg" alt="Picture of Pastor William Burks"/>
                    <figcaption>
                        <strong>Pastor William Burks</strong><br/> Patient Advisory Committee Member
                    </figcaption>
                </figure>
            </div>
            <div class="column small-12 medium-9">
                <p>I was diagnosed with diabetes approximately 3 years ago, and to say the least it was devastating to get that information. But I decided to not let diabetes rule my life, but for me to take control of the situation.</p>

                    <p>Diabetes, to me, is a journey, where in you control how far you go. You control what you do on the journey. So I have been very, very blessed in that I do check my glucose, on a daily basis, I check it periodically after I've eaten certain foods. Because I do not, I don't eat a lot of things that the average person eats. I do consume a lot of vegetables. I eat a lot of green vegetables. I eat a lot of fruit. I am specific about which fruits I eat and what time I eat them. I drink at least 96 ounces of water on a daily basis. I want you to know that this not a death sentence. Being diabetic is not a death sentence. You can do something about your condition. Make up your mind, get you a partner, someone who can work with you. Someone who will help keep you encouraged on your journey. And then what you can do is take charge of the situation. Eat right, get plenty rest, plenty of exercise, and enjoy your life—those things really make the differences in how you do as a diabetic.
                </p>
            </div>
        </div>
    </div>
</div>
<?php include("template-bottom.php"); ?>
